 ...
 */
  ...
  if ($balance > 0 ){//&& $balance >= $mbrMaxFines[$mbr->getClassification()]) {
    $balText = 'Php '.number_format($balance,2);
...
?>
  <tr>
    ...
    <td class="primary" valign="top" >
    <?php
		$chkdate = strtotime($biblio->getStatusBeginDt()); 
		$due = strtotime($biblio->getDueBackDt());
		
	if($chkdate!=strtotime(date('Y-m-d'))&&strtotime(date('Y-m-d'))<=$due)
	{
	?>
