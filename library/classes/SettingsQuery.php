...
class SettingsQuery extends Query {
   ...
   */
  function update($set) {
    $sql = $this->mkSQL("update ob_settings set "
  ...
  function updateTheme($themeId) {
    $sql = $this->mkSQL("update ob_settings set themeid=%N", $themeId);
    return $this->_query($sql, "Error updating library theme in use");
  }

  function getPurgeHistoryAfterMonths($query) {
    $sql = "select purge_history_after_months from ob_settings";
    $rows = $query->exec($sql);
    if (count($rows) != 1) {
