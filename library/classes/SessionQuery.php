...
class SessionQuery extends Query {
  ...
  function validToken($userid, $token) {
    $sql = $this->mkSQL("select last_updated_dt, token from ob_session "
  ...
  function getToken($userid) {
     ...
     **************************************************************************/
    $sql = $this->mkSQL("delete from ob_session where userid = %N"
                        . " and last_updated_dt < date_sub(sysdate(), interval %N minute)",
    ...
    $sql = $this->mkSQL("insert into ob_session "
                        . "values (%N, sysdate(), %N) ",
                        $userid, $token);
  ...
  function _updateToken($token) {
    $sql = $this->mkSQL("update ob_session set last_updated_dt=sysdate() "
                        . "where token = %Q ", $token);
