...
class BiblioCopyQuery extends Query {
  ...
  function doQuery($bibid,$copyid) {
    ...
    $sql = $this->mkSQL("select ob_biblio_copy.*, "
                        . " greatest(0,to_days(sysdate()) - to_days(ob_biblio_copy.due_back_dt)) days_late "
                        . "from ob_biblio_copy "
                        . "where ob_biblio_copy.bibid = %N"
                        . " and ob_biblio_copy.copyid = %N",
                        $bibid, $copyid);
  ...
  function queryByBarcode($barcode) {
    ...
    $sql = $this->mkSQL("select ob_biblio_copy.*, "
                        . "greatest(0,to_days(sysdate()) - to_days(ob_biblio_copy.due_back_dt)) days_late "
                        . "from ob_biblio_copy where ob_biblio_copy.barcode_nmbr = %Q",
                        $barcode);
  ...
  function execSelect($bibid) {
    ...
    $sql = $this->mkSQL("select ob_biblio_copy.* "
                        . ",greatest(0,to_days(sysdate()) - to_days(ob_biblio_copy.due_back_dt)) days_late "
                        . "from ob_biblio_copy where ob_biblio_copy.bibid = %N",
                        $bibid);
  ...
  function _dupBarcode($barcode, $bibid=0, $copyid=0) {
    $sql = $this->mkSQL("select count(*) from ob_biblio_copy "
  ...
  function nextCopyid($bibid) {
    $sql = $this->mkSQL("select max(copyid) as lastNmbr from ob_biblio_copy "
                        . "where ob_biblio_copy.bibid = %Q",
                        $bibid);
  ...
  function insert($copy) {
    ...
    $sql = $this->mkSQL("insert into ob_biblio_copy values (%N"
  ...
  function update($copy, $checkout=FALSE) {
    ...
    $sql = $this->mkSQL("update ob_biblio_copy set "
                        . "status_cd=%Q, "
    ...
    if ($checkout){
      ...
      if ($copy->getMbrid() != "") {
        $sql .= $this->mkSQL("student_id=%N, ", $copy->getMbrid());
      } else {
        $sql .= "student_id=null, ";
      }
    }
  ...
  function delete($bibid,$copyid=0) {
    $sql = $this->mkSQL("delete from ob_biblio_copy where bibid = %N", $bibid);
    if ($copyid > 0) {
  ...
  function _getCollectionInfo($bibid) {
    ...
    $sql = $this->mkSQL("select collection_cd from ob_biblio where bibid = %N",
                        $bibid);
    ...
    $sql = $this->mkSQL("select * from ob_collection_dm where code = %N",
                        $collectionCd);
  ...
  function checkin($massCheckin,$bibids,$copyids) {
    $sql = $this->mkSQL("update ob_biblio_copy set "
                        ...
                        . " due_back_dt=null, student_id=null "
                        . "where status_cd=%Q ",
  ...
  function _getCheckoutPrivs($bibid, $classification) {
    $sql = $this->mkSQL("select ob_checkout_privs.* "
                        . "from ob_biblio, ob_checkout_privs "
                        . "where bibid=%N and classification=%N "
                        . "and ob_biblio.material_cd=ob_checkout_privs.material_cd ",
  ...
  function hasReachedCheckoutLimit($mbrid,$classification,$bibid) {
    ...
    $sql = $this->mkSQL("select count(*) row_count from ob_biblio_copy, ob_biblio"
                        . " where ob_biblio_copy.bibid = ob_biblio.bibid"
                        . " and ob_biblio_copy.student_id = %N"
                        . " and ob_biblio.material_cd = %N",
