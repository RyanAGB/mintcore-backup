...
class MaterialFieldQuery extends Query {
  function get($materialCd) {
    $sql = 'SELECT * FROM ob_material_usmarc_xref '
  ...
  function insert($record) {
    $sql = 'INSERT INTO ob_material_usmarc_xref '
  ...
  function update($record) {
    $sql = $this->mkSQL('UPDATE ob_material_usmarc_xref SET '
  ...
  function delete($id) {
    $sql = 'DELETE FROM ob_material_usmarc_xref '
           . $this->mkSQL('WHERE xref_id=%N ', $id);
    $this->exec($sql);
  }
}
