...
class BiblioQuery extends Query {
  ...
  function doQuery($bibid) {
    ...
    $sql = $this->mkSQL("select ob_biblio.*"
                        . "from ob_biblio left join tbl_employee emp "
                        . "on ob_biblio.last_change_userid = emp.user_id "
                        . "where ob_biblio.bibid = %N ", $bibid);
     ...
     ***********************************************************/
    # setting query that will return all the data in biblio
    $sql = $this->mkSQL("select ob_biblio_field.* "
                        . "from ob_biblio_field "
                        . "where ob_biblio_field.bibid = %N "
  ...
  function _dupBarcode($barcode, $bibid=0) {
    $sql = $this->mkSQL("select count(*) from ob_biblio "
  ...
  function insert($biblio) {
    ...
    $sql = $this->mkSQL("insert into ob_biblio values(null, sysdate(), sysdate(), "
  ...
  function update($biblio) {
    ...
    $sql = $this->mkSQL("update ob_biblio set last_change_dt = sysdate(), "
    ...
    foreach ($biblioFlds as $key => $value) {
      if ($value->getFieldData() == "") {
        ...
        if ($value->getFieldid() == "") {
          ...
          $sql = $this->mkSQL("delete from ob_biblio_field "
                                . "where bibid=%N and fieldid=%N ",
                                $value->getBibid(), $value->getFieldid());
        }          
      } elseif ($value->getFieldid() == "") {
        // new value
        $sql = $this->mkSQL("insert into ob_biblio_field "
        ...
        // existing value
        $sql = $this->mkSQL("update ob_biblio_field set field_data = %Q "
                            . "where bibid=%N and fieldid=%N ",
  ...
  function delete($bibid) {
    $sql = $this->mkSQL("delete from ob_biblio_field where bibid = %N ", $bibid);
    if (!$this->_query($sql, $this->_loc->getText("biblioQueryDeleteErr"))) {
      return false;
    }
    $sql = $this->mkSQL("delete from ob_biblio where bibid = %N ", $bibid);
