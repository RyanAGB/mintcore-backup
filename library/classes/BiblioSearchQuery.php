...
class BiblioSearchQuery extends Query {
  ...
  function search($type, &$words, $page, $sortBy, $opacFlg=true) {
    ...
    $join = "from ob_biblio left join ob_biblio_copy on ob_biblio.bibid=ob_biblio_copy.bibid ";
    ...
    if ((sizeof($words) == 0) || ($words[0] == "")) {
      ...
      if ($type == OBIB_SEARCH_BARCODE) {
        $criteria = $this->_getCriteria(array("ob_biblio_copy.barcode_nmbr"),$words);
      } elseif ($type == OBIB_SEARCH_AUTHOR) {
        $join .= "left join ob_biblio_field on ob_biblio_field.bibid=ob_biblio.bibid "
                 . "and ob_biblio_field.tag='700' "
                 . "and (ob_biblio_field.subfield_cd='a' or ob_biblio_field.subfield_cd='b') ";
        $criteria = $this->_getCriteria(array("ob_biblio.author","ob_biblio.responsibility_stmt","ob_biblio_field.field_data"),$words);
      } elseif ($type == OBIB_SEARCH_SUBJECT) {
        $criteria = $this->_getCriteria(array("ob_biblio.topic1","ob_biblio.topic2","ob_biblio.topic3","ob_biblio.topic4","ob_biblio.topic5"),$words);
      } else {
        $criteria = $this->_getCriteria(array("ob_biblio.title"),$words);
      }
    ...
    $sql = "select ob_biblio.* ";
    $sql .= ",ob_biblio_copy.copyid ";
    $sql .= ",ob_biblio_copy.barcode_nmbr ";
    $sql .= ",ob_biblio_copy.status_cd ";
    $sql .= ",ob_biblio_copy.due_back_dt ";
    $sql .= ",ob_biblio_copy.student_id ";
    $sql .= $join;
    $sql .= $criteria;
  ...
  function doQuery($statusCd,$mbrid="") {
    ...
    $sql .= ",ob_biblio_copy.barcode_nmbr ";
    $sql .= ",ob_biblio_copy.status_cd ";
    $sql .= ",ob_biblio_copy.status_begin_dt ";
    $sql .= ",ob_biblio_copy.due_back_dt ";
    $sql .= ",ob_biblio_copy.student_id ";
    $sql .= ",ob_biblio_copy.renewal_count ";
    $sql .= ",greatest(0,to_days(sysdate()) - to_days(ob_biblio_copy.due_back_dt)) days_late ";
    $sql .= "from ob_biblio, ob_biblio_copy ";
    $sql .= "where ob_biblio.bibid = ob_biblio_copy.bibid ";
    if ($mbrid != "") {
        $sql .= $this->mkSQL("and ob_biblio_copy.student_id = %N ", $mbrid);
    }
    $sql .= $this->mkSQL(" and ob_biblio_copy.status_cd=%Q ", $statusCd);
    $sql .= " order by ob_biblio_copy.status_begin_dt desc";
