...
$pagination = 'Student > School Calendar';
...
$filter_schoolterm	= $_REQUEST['f_term_id'];
...
if($action == 'save')
{
	...
	else
	{
		...
		{
			// [+] STORE SYSTEM LOGS
			$param = array(getSectionNo($schedule_id),getSubjNameBySchedId($schedule_id),getSubjCodeBySchedId($schedule_id));
			storeSystemLogs(MSG_PROF_CALENDAR_FOR_STUDENTS,$param,'','Y','Y');			
		// [-] STORE SYSTEM LOGS	
			
			$studs = getStudentEnrolledBySchedule($schedule_id,$filter_schoolterm);
			if(count($studs)>0)
			{
				foreach($studs as $stud)
				{
					//notification(getStudentUser($stud),MSG_MODIFIED_PROF_CALENDAR_FOR_STUDENTS,$param,getProfessorUserID(getSchedProf($schedule_id)));
					//notification(getParentUser($stud),MSG_MODIFIED_PROF_CALENDAR_FOR_STUDENTS,$param,getProfessorUserID(getSchedProf($schedule_id)));
				}
			}
			...
			{
				//notification($admin,MSG_PROF_CALENDAR_FOR_STUDENTS,$param,getProfessorUserID(getSchedProf($schedule_id)));					
			}
		
			echo '<script language="javascript">alert("Successfully Added!");window.location =\'index.php?comp=com_pr_school_calendar\';</script>';
		}
	}	
}
	...
	else
	{
		if (storedModifiedLogs(tbl_school_calendar, $id)) {
			 $sql = "UPDATE tbl_school_calendar SET 
					...
					schedule_id  =".GetSQLValueString($schedule_id,"text").",
			...
			if(mysql_query ($sql))
			{
				// [+] STORE SYSTEM LOGS
				$param = array(getSectionNo($schedule_id),getSubjNameBySchedId($schedule_id),getSubjCodeBySchedId($schedule_id));
				storeSystemLogs(MSG_MODIFIED_PROF_CALENDAR_FOR_STUDENTS,$param,'','Y','Y');			
			// [-] STORE SYSTEM LOGS	
				...
				foreach($admins as $admin)
				{
					notification($admin,MSG_MODIFIED_PROF_CALENDAR_FOR_STUDENTS,$param,getProfessorUserID(getSchedProf($schedule_id)));					
				}	
			
				echo '<script language="javascript">alert("Successfully Updated!");window.location =\'index.php?comp=com_pr_school_calendar\';</script>';
			}
		}	
...
$content_template = 'components/block/blk_com_pr_school_calendar.php';
?>