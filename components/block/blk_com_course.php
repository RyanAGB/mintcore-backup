...
$(document).ready(function(){ 
	...
	$('#filter_college').change(function(){
		$('#filtercol').val($('#filter_college').val());
		updateList();
	});
	// Initialize the list
	<?php
...
function updateList(pageNum)
{
	...
	if($('#filtercol').val() != '' && $('#filtercol').val() != undefined)
	{
		param = param + '&filter_college='+$('#filtercol').val();
	}
	...
	$('#box_container').load('ajax_components/ajax_com_course.php?param=1' + param, null);
}
...
<h2><?=$page_title?></h2>
<ul class="tabs">
<li id="course_list" <?=$view=='list'?'class="active"':''?>><a href="#" title="Course List"><span>Course List</span></a></li>
<?php 
if($canEdit=='Y')
{
?>
...
<?php
    ...
    <div class="filter">
    Filter By: College <select name="filter_college" id="filter_college" class="txt_150" >
    
                    <option value="" >-Select-</option>
                     <?=generateCollege($filtercol)?>  
                </select>
	...
	<div class="formview">
        ...
        <fieldset>
           <legend><strong>Course Information</strong></legend>
            ...
            <span ><input class="txt_100 {required:true}" title="Course Code field is required" name="course_code" type="text" value="<?=$course_code?>" id="course_code" />
            </span><br class="hid" />
            <label>Course Name:</label>
            <span class="small_input"><input class="txt_250 {required:true}" title="Course Name field is required" name="course_name" type="text" value="<?=$course_name?>" id="course_name" />
            </span><br class="hid" />
            <label>Description</label>
            <span><input class="txt_250" name="description" type="text" value="<?=$description?>" id="description" />
            </span><br class="hid" />
	
			 <label>College:</label>
            <span>
                <select name="college_id" id="college_id" class="txt_250 {required:true}" title="College Name field is required" >
        
                    <option value="" selected="selected">Select College</option>
                    <?=generateCollege($college_id)?>
...
<input type="hidden" name="filtercol" id="filtercol" value="<?=$filtercol?>" />
