...
function updateList(pageNum)
{
	...
	if($('#cal_rows').val() != '' && $('#cal_rows').val() != undefined )
	{
		param = param + '&cal_rows=' + $('#cal_rows').val() + param;
	}
...
?>
     <div id="pageRows">
        ...
        <span>Professor Calendar</span>
        <select name="cal_rows" id="cal_rows">
          <option value="Y"<?=$_SESSION[CORE_U_CODE]['cal_rows']=='Y'||$_REQUEST['cal_rows']=='Y' ? 'selected=selected':''?>>On</option>
          <option value="N"<?=$_SESSION[CORE_U_CODE]['cal_rows']=='N'||$_REQUEST['cal_rows']=='N' ? 'selected=selected':''?>>Off</option>            
        </select>
    </div>  
<?php
	} // end of page rows
	...
	<div class="formview">
        ...
        <fieldset>
           <legend><strong>School Calendar Information</strong></legend>
            ...
            <span>
            <textarea name="description" cols="60" rows="7" class="txt_250 {required:true}" id="description" title="Description field is required"><?=str_replace('<br>', "\n", $description)?></textarea>
    		</span><br class="hid" />
