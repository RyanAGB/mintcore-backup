...
function updateList(pageNum)
{
	...
	$('#box_container').load('ajax_components/ajax_com_payment_types.php?param=1' + param, null);
}
...
<h2><?=$page_title?></h2>
<ul class="tabs">
<li id="room_list" <?=$view=='list'?'class="active"':''?>><a href="#" title="Payment Type List"><span>Payment Types List</span></a></li>
	...
	<div class="formview">
        ...
        <fieldset>
           <legend><strong>Payment Information</strong></legend>
            ...
            <label>Library Payment:</label>
            <span >
                <select name="is_library" id="is_library" class="txt_150 {required:true}" title="Publish field is required" >
                    <option value="" selected="selected">Select</option>
                    <option value="Y" <?=$is_library== "Y"?'selected = "selected"':''?> >Yes</option>
                    <option value="N" <?=$is_library== "N"?'selected = "selected"':''?> >No</option>
                </select>    
            </span><br class="hid" /> 
            <label>Publish:</label>
            <span >
