...
function updateList(pageNum)
{
	...
	$('#box_container').load('ajax_components/ajax_com_school_report.php?param=1' + param, null);
}
...
<h2><?=$page_title?></h2>
<ul class="tabs">
<li id="room_list" <?=$view=='list'?'class="active"':''?>><a href="#" title="Building List"><span>Report List</span></a></li>
<?php
if($canEdit=='Y')
{
?>
...
?>
	<div class="formview">
        ...
        <fieldset>
           <legend><strong>School Report Information</strong></legend>
            <label>Title:</label>
            <span ><input class="txt_300 {required:true}" title="Title field is required" name="report_name" type="text" value="<?=$report_name?>" id="report_name" />
            </span><br class="hid" />
            <label>Classification:</label>
            <span><select name="classification" id="classification" class="txt_150 {required:true}">
            <option value="">Select</option>
              <option value="S" <?=$classification=='S'?'selected="selected"':''?>>Student</option>
              <option value="E" <?=$classification=='E'?'selected="selected"':''?>>Employee</option>
              <option value="C" <?=$classification=='C'?'selected="selected"':''?>>Cashier</option>
            </select>
            </span><br class="hid" />
