...
$(function(){
	...
	// Dialog Link
	$('#dialog_sub').click(function(){
		var param = $(this).attr("returnComp");
		$('#dialog').load('lookup/lookup_com_subject.php?comp='+param, null);
		$('#dialog').dialog('open');
		return false;
	});
...
function updateList(pageNum)
{
	...
	{
		param = param + '&filter_dept='+$('#filter_dept').val();
	}	
	...
	$('#box_container').load('ajax_components/ajax_com_subject.php?param=1' + param, null);
}
...
<h2><?=$page_title?></h2>
<ul class="tabs">
<li id="room_list" <?=$view=='list'?'class="active"':''?>><a href="#" title="Room List"><span>Subject List</span></a></li>
...
<?php
	...
	?>
         ...
         <div class="filter">
    Filter By: Department <select name="filter_dept" id="filter_dept" class="txt_150" >
    
                    <option value="" >-Select-</option>
                     <?=generateDepartment($_REQUEST['filter_dept'])?>  
                </select>
	...
	<div class="formview">
        ...
        <fieldset>
           <legend><strong>Subject Information</strong></legend>
            ...
            <span ><input class="txt_100 {required:true}" title="Subject Code field is required" name="subject_code" type="text" value="<?=$subject_code?>" id="subject_code" />
            </span><br class="hid" />
            <label>Subject Name:</label>
            <span ><input class="txt_250 {required:true}" title="Subject Name field is required" name="subject_name" type="text" value="<?=$subject_name?>" id="subject_name" />
            </span><br class="hid" />
            <br class="hid" />
            <label>Department Name:</label>
            <span>
            <div style=" float:left">
            <input class="txt" name="department_id" type="hidden" value="<?=$department_id?>" id="department_id" readonly="readonly" />
            <input class="txt_250 {required:true}" title="Department Name field is required" name="department_name_display" type="text" value="<?=$department_name_display?>" id="department_name_display" readonly="readonly" />
            </div>
            <div style="width:50px; float:left">
            <a href="#" class="lookup_button" id="dialog_sub" returnComp="<?=$_REQUEST['comp']?>"><span>...</span></a><br class="hid" />
            </div>
        	</span><br class="hid" />          
            ...
            <!-- LIST LOOK UP-->
            <div id="dialog" title="Subject List">
                Loading...
            </div><!-- #dialog -->
    
			  <label>Subject Type:</label>
                <span>
				<select name="subject_type" id="subject_type" class="txt_150 {required:true}" title="Subject Type field is required" >
                    <option value="" selected="selected">Select</option>
                    <option value="Lec" <?=$subject_type== "Lec"?'selected = "selected"':''?> >Lec</option>
                    <option value="Lab" <?=$subject_type== "Lab"?'selected = "selected"':''?> >Lab</option>
                </select>    	
                </span>
                <br class="hid" />
