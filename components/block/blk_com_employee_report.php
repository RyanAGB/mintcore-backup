...
<?php
		...
		$sql = "SELECT * FROM tbl_report_fields WHERE category = 'E' AND publish ='Y' ORDER BY value_type";						
...
function getValueMasking(fieldname,fieldvalue,obj)
{
	$.ajax({
		...
		url: "ajax_components/ajax_com_employee_report_value_masking.php",
...
function updateList(pageNum)
{
	...
	$('#box_container').load('ajax_components/ajax_com_employee_report.php' + param, null);
}
              ...
              <tr>
                ...
                <td style="padding-left:10px; height:30px;">
                    <select name="filter_fieldname" id="filter_fieldname" class="txt_200">
                        ...
                        <?=generateEmployeeReportFields($filter_fieldname)?>
                    </select>        </td>
              </tr>
              <tr>
                <td>Filter Way</td>
