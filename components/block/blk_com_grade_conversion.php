...
$(document).ready(function(){  
	...
	$('#edit_item').click(function(){
		...
		else
		{
			...
			clearTabs();
			$('#grade_conversion_list').addClass('active');
			updateList();
			return false;
		}
	});
	
	$('#grade_conversion_list').click(function(){
		clearTabs();
		$('#grade_conversion_list').addClass('active');
		$('#view').val('list');
		$('#action').val('list');
...
function updateList(pageNum)
{
	...
	$('#box_container').load('ajax_components/ajax_com_grade_conversion.php?param=1' + param, null);
}
...
<h2><?=$page_title?></h2>
<ul class="tabs">
<li id="grade_conversion_list" <?=$view=='list'?'class="active"':''?>><a href="#" title="College List"><span>Grade Conversion List</span></a></li>
...
?>
	<div class="formview">
        ...
        <fieldset>
           <legend><strong>Grade Conversion Information</strong></legend>
            <label>Floor Grade:</label>
            <span>
            <input class="txt_50 {required:true}" title="Floor Grade field is required" name="floor_grade" type="text" value="<?=$floor_grade?>" id="floor_grade" />
    		</span><br class="hid" />
			<label>Ceiling Grade:</label>
            <span>
            <input class="txt_50 {required:true}" title="Ceiling Grade field is required" name="ceiling_grade" type="text" value="<?=$ceiling_grade?>" id="ceiling_grade" />
    		</span><br class="hid" />
			<label>Grade Code:</label>
            <span>
            <input class="txt_50 {required:true}" title="Grade Code field is required" name="grade_code" type="text" value="<?=$grade_code?>" id="grade_code" />
    		</span><br class="hid" />
            <!--
			<label>Letter:</label>
            <span>
            <input class="txt_50" name="letter" type="text" value="<?=$letter?>" id="letter" />
    		</span><br class="hid" />
            -->
            <label>Description:</label>
            <span>
            <textarea name="description" cols="50" rows="8" class="txt_250 {required:true}" id="description" title="Description field is required"><?=$description?></textarea>
    		</span><br class="hid" />
        ...
        <fieldset>
           <legend><strong>System Information</strong></legend>
			 <label>Is Grade Passing:</label>
            <span >
                <select name="is_grade_passing" id="is_grade_passing" class="txt_150 {required:true}" title="Is Grade Passing field is required" >
				<option value="" selected="selected">Select</option>
                <option value="Y" <?=$is_grade_passing== "Y"?'selected = "selected"':''?> >Yes</option>
                <option value="N" <?=$is_grade_passing== "N"?'selected = "selected"':''?> >No</option>
                </select>    
            </span><br class="hid" />            
            <!--<label>Publish:</label>
            <span >
