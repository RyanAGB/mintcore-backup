...
$(function(){
	...
	$('.lookup_button').click(function(){
		...
		$('#dialog').load('lookup/lookup_com_department.php?comp='+param, null);
		$('#dialog').dialog('open');
		return false;
	});
...
function updateList(pageNum)
{
	...
	$('#box_container').load('ajax_components/ajax_com_department.php?param=1' + param, null);
}
...
<h2><?=$page_title?></h2>
<ul class="tabs">
<li id="room_list" <?=$view=='list'?'class="active"':''?>><a href="#" title="Room List"><span>Department List</span></a></li>
<?php
if($canEdit =='Y')
{
?>
	...
	<div class="formview">
        ...
        <fieldset>
           <legend><strong>Department Information</strong></legend>
            ...
            <span ><input class="txt_100 {required:true}" title="Department Code field is required" name="department_code" type="text" value="<?=$department_code?>" id="department_code" />
            </span><br class="hid" />
            <label>Department Name:</label>
            <span ><input class="txt_100 {required:true}" title="Department Name field is required" name="department_name" type="text" value="<?=$department_name?>" id="department_name" />
            </span><br class="hid" />
            <br class="hid" />
            ...
            <input class="txt_250 " title="Department Head field is required" name="department_head_display" type="text" value="<?=$department_head_display?>" id="department_head_display" readonly="readonly" />
            </div>
            <div style="width:50px; float:left">
            <a href="#" class="lookup_button" returnComp="<?=$_REQUEST['comp']?>"><span>...</span></a><br class="hid" />
            </div>
        	</span><br class="hid" /> 
            </span><br class="hid" />           
