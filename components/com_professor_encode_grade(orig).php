...
if($action == 'save')
{
	...
	foreach($student_id as $student)
	{
		if(checkIfGradeIsPass(getStudentFinalGrade($student,$sched_id,$term_id)) == 'Y')
		{
			$remarks = 'P';
		}
		else
		...
		if(checkIfAllPeriodExistPerStud($sched_id,$student)  && !checkIfStudentFinalGradeExistPerTerm(getSchedSubjectId($sched_id),getTermIdBySchedId($sched_id),$student))
		{
			...
			(
				...
				".GetSQLValueString(encrypt(getStudentFinalGrade($student,$sched_id,$term_id)),"text").",
				".GetSQLValueString('S',"text").",
				".GetSQLValueString($remarks,"text").",
				".GetSQLValueString(getGradeConversionId(getStudentFinalGrade($student,$sched_id,$term_id)),"text").",
				".time().",
				".USER_ID."		
			)";
		...
		else if(checkIfAllPeriodExistPerStud($sched_id,$student))
		{
			...
			$sql = "UPDATE tbl_student_final_grade SET
						final_grade =".GetSQLValueString(encrypt(getStudentFinalGrade($student,$sched_id,$term_id)),"text").",
						type  =".GetSQLValueString('S',"text").",				
						remarks = ".GetSQLValueString($remarks,"text").",
						grade_conversion_id = ".GetSQLValueString(getGradeConversionId(getStudentFinalGrade($student,$sched_id,$term_id)),"text")."
					WHERE 
