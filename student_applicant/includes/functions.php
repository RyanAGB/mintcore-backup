...
/* [+] ALL THE GENERATE SELECT BOX FUNCTION */
	...
	{		
...

			{
...

				{
...

					{
						...
						list($fst,$snd,$last_num) = explode('-',$row['student_number']);
			...
			return $year.$term.'-'.getStudentCollegeCode($course).'-'.$last;
	...
	function generateFeeType($selected_id)

	{
...

		{
			...
			$arr_str[] = '<option value="'.$row['id'].'" '.$selected.' >'.$row['fee_type_name'].'</option>';
	...
	function generateScheme($selected_id)

	{
		...
		$sql = "SELECT * FROM tbl_payment_scheme WHERE term_id = ".CURRENT_TERM_ID." AND publish ='Y'";						
...
/* [+] ALL THE GET VALUE FUNCTION */
...

	{
...

		{
			...
			$exam = getMonthName($date[1]).' '.$date[2].', '.$date[0];
...

	{
...

		{
			...
			$yr_level = getStudentCurrentYearLevel($id);
...

	{
			...
			$sql = "SELECT * FROM tbl_payment_method WHERE name = 'Cheque' OR name = 'Check'";
...

	{
...

		{
			...
			$sql = "SELECT subject_name FROM tbl_subject WHERE id = $id";
...

	{
...

		{
			...
			$sql = "SELECT stat.student_id FROM 

								tbl_student_enrollment_status stat, 

								tbl_student stud,

								tbl_block_section blck
							...
							WHERE 
								...
								stat.block_id = blck.id AND

								blck.course_id = $course_id AND

								blck.course_id = $year_lvl AND

								stud.course_id = $course_id AND 

								blck.term_id = ".$term_id;
...

			{
				...
				if(!in_array($row['student_id'],$stud_arr) && $ctr > 0)

				{

					$stud_arr[] = $row['student_id'];
	...
	{ 
...

		{
			...
			if($row['school_term'] == 'First Term') 		$school_term =  1;

			else if($row['school_term'] == 'Second Term') $school_term =  2;

			else if($row['school_term'] == 'Third Term') $school_term =  3;

			else if($row['school_term'] == 'Fourth Term') $school_term =  4;	
...

	{
...

		{
			...
			$sql = "SELECT * FROM tbl_fee_type WHERE id = ".$fee_type;
			
			$query = mysql_query($sql);	

			$row = mysql_fetch_array($query);

			return $row['fee_type_name'];
	...
	function getFeeTypeId($fee_type)
	...
	
	{
		...
		
		{
			...
			$sql = "SELECT * FROM tbl_extra_fee WHERE scheme_id = ".$id;
...

	{
...

		{
			...
			$term = getPreviousTermByYear($term_id);
					...
					$sql = "SELECT * FROM tbl_school_fee WHERE publish =  'Y' AND term_id=" .$term;
					...
					$sqlo = "SELECT sum(school_fee.amount) as total_other_fee 

					FROM tbl_school_fee as school_fee, tbl_student_fees as student_fee 

					WHERE student_fee.fee_id = school_fee.id 

					AND school_fee.id =student_fee.fee_id 

					AND (school_fee.fee_type <> 'perunitlec' 

					AND school_fee.fee_type <> 'perunitlab') 

					AND school_fee.term_id=".$term." AND school_fee.publish = 'Y' 

					AND student_fee.is_removed = 'N' AND student_fee.student_id =".$id;
					...
					$sub_mis_total = $rowo['total_other_fee'];;
					...
					$sql_lec = "SELECT * FROM tbl_school_fee WHERE fee_type = 'perunitlec' AND publish =  'Y' AND term_id=" .$term;
					...
					$sql_lab = "SELECT * FROM tbl_school_fee WHERE fee_type = 'perunitlab' AND publish =  'Y' AND term_id=" .$term;
					...
					//TOTAL PAYMENT						
						...
						$total_payment = getTotalPaymentOfStudent($id,$term); 
...

	{
...

		{
			...
			while ($row = mysql_fetch_array($query))
			{
				$arr_prereq[] = $row['coreq_subject_id'];
			}
...

	{
...

		{
			...
			{				
				...
				foreach(checkIfSubjectIsCoreq($row['subject_id']) as $cor)
...

		{
...

			{
...

				{
					...
					return decrypt($row['altered_grade']);
...

			{
...

				{
...

					{
						...
						$grade = decrypt($row['altered_grade']);
	...
	function getGradeConversionId($grade)

	{
...

		{
			...
			$sql = "SELECT id FROM tbl_grade_conversion 
					...
					WHERE '".round($grade)."' BETWEEN floor_grade AND ceiling_grade";
	...
	function getStudentTotalFee($id)
...

	{
...

		{
			...
			$sql = "SELECT sum(amount) as total_amount, sum(quantity) as total_quantity 
...

	{
...

		{
			...
			$sql = "SELECT sum(quantity) as total_unit 
...

	{
...

		{
				...
				$sql = "SELECT * FROM tbl_student_fees WHERE is_removed = 'Y' 

							AND student_id = ".$student_id." AND subject_id = ".$id;
...

	{
...

		{
			...
			$sql = "SELECT * FROM tbl_student_fees WHERE

						student_id = ".$id." AND term_id = ".$term;
...
/* [+] ALL SYSTEM FUNCTION */
	...
	function generatePassword($length = 8, $letters = 

	'1234567890qwertyuiopasdfghjklzxcvbnmABCDEFGHIJKLMNOPQRSTUVWXYZ') 
...

	{
...

		{
			...
			$sql = "SELECT * FROM tbl_access WHERE 

				id= ".GetSQLValueString($access_id,"text")
...

	{
...

		{
			...
			$sql = "SELECT * FROM tbl_student_grade WHERE 

			schedule_id = $sched_id AND student_id = $student_id AND term_id = $term_id ";
...

	{
...

		{
			...
			$sql_logs = "SELECT * FROM tbl_session_logs WHERE 

					user_id= ".GetSQLValueString($row['id'],"int")
	...
	function storeStudentFee($student_id)
	...
	function storeStudentOtherFee($student_id)
...

	{
		...
		$avail = abs($student - $reserved);
			...
			$sql = "UPDATE tbl_schedule SET number_of_available = ".$avail." 
...
/* [+] ALL VALIDATOR FUNCTION */
...

	{
		...
		$sql_ps = "SELECT * FROM tbl_payment_scheme as pay_scheme, 

								 tbl_payment_scheme_details as pay_scheme_det
...

		{
			...
			$sql = "SELECT count(*) FROM tbl_payment_scheme_details 
							...
							payment_name = ".GetSQLValueString($payment_name,"text") .
...

	{
		...
		$sqlSub = "SELECT * FROM tbl_curriculum_subject 

							WHERE curriculum_id = ".$row['id'];
	...
	*/
...

	{
		...
		/* IF subject duplicates validation */
			...
			$sql = "SELECT subject_id FROM tbl_schedule 
...

	{
...

		{
			...
			$sql = "SELECT count(*) FROM 
							...
							tbl_student_reserve_subject as stud_reserve
...

	{
...

		{
			...
			$sql = "SELECT count(*) FROM 
							...
							sched.id = stud_sched.schedule_id 

							AND sched.term_id = ".GetSQLValueString($term_id,"int").

							" AND stud_sched.schedule_id = $id"
...

	{
...

		{
			 ...
			 $sql = "SELECT count(*) FROM tbl_exam_date
							...
							entrance_date = ".GetSQLValueString($start_date,"date") . 
...

		{
...

			{
				...
				$sql = "SELECT count(*) FROM tbl_exam_date
								...
								entrance_date = ".GetSQLValueString($date,"date") .
...

	{
...

		{
			...
			if((isset($period_name) && $period_name!='') ||(isset($start_of_submission) && $start_of_submission!='') || (isset($end_of_submission) && $end_of_submission!='') ||

				(isset($term_id ) && $term_id !='') ||(isset($school_year_id) && $school_year_id!=''))
...

	{
...

		{
			...
			$sql = "SELECT count(*) FROM tbl_discount as discount, 

							tbl_student_payment as stud_payment 
...

		{
...

			{
				...
				$sql = "SELECT count(*) FROM tbl_room 
								...
								room_no = ".GetSQLValueString($room_no,"text"). " AND
...

	{
...

		{
			...
			$sql = "SELECT * FROM tbl_employee_type 

					WHERE type_title = '".$title."'";
...
function checkBalanceCarried($id,$prev_term)
	...
	function checkIfRoomIsAvail($room_id, $term_id, $days, $start, $end, $id ="")
...

		{
			...
			{								
				...
				{					
					...
					AND ".$days[$x]."_time_to )) AND term_id =" . $term_id . " 
...

		{
...

			{
				...
				{		
					...
					AND ".$day[$x]."_time_to ))	AND term_id =" . $term_id . " 
...

{
	...
	$sql = "SELECT sum(amount) as payment
...

{
...

	{
		...
		$sql_student = "SELECT * FROM tbl_school_fee WHERE term_id = ".CURRENT_TERM_ID."
...
function checkFeeTypeEditable($id)

{
...

	{
		...
		$sql_student = "SELECT * FROM tbl_school_fee WHERE fee_type = ".$id." AND term_id = ".CURRENT_TERM_ID;
...
function checkExtraFeeEditable($id)

{
...

	{
		...
		$sql_student = "SELECT * FROM tbl_student_fees WHERE fee_id = ".$id." AND term_id = ".CURRENT_TERM_ID;
...
function checkSchemeEditable($id)

{
...

	{
		...
		$sql_sched= "SELECT * FROM tbl_student_payment pay, tbl_payment_scheme scheme 	
						...
						" AND scheme.term_id = ".CURRENT_TERM_ID;
