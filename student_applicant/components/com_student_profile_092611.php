...
if($action == 'save')
{
	if($lastname==''||$firstname==''||$middlename==''||$gender==''||$civil_status==''||$citizenship==''||$birth_date==''||$birth_place==''||$tel_number==''||$mobile_number==''||$email==''||$guardian_name==''||$guardian_relation==''||$country==''||$term_id==''||$course_id==''||$exam_date==''||$admission_type==''||$year_level=='')
	{
	...
	*/
	...
	{		
			...
			{	
				...
				$contents .= "Your selected examination is on ".getExamDate($exam_date).", you should bring a valid ID with photo, preferably your high school ID and your Admission Requirements as ".getStudentAdmissionType($admission_type).". \n";	
