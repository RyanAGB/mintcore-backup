...
?>
	...
	$(function(){
		...
		$('#pdf').click(function() {
			var w=window.open ("pdf_reports/rep108.php?id="+<?=USER_STUDENT_ID?>+"&trm="+<?=$term_id?>+"&met=grade"); 
			return false;
		});
		...
		$('#email').click(function() {
			...
			if(confirm("Are you sure you want to email this file to this student?"))
			{
				$.ajax({
					type: "POST",
					data: "id="+<?=USER_STUDENT_ID?>+"&trm="+<?=$term_id?>+"&met=grade&email=1",
...
<?php	
	...
	$sql_pagination = "SELECT 
							...
							WHERE 
								...
								stud_sched.student_id = ".USER_STUDENT_ID
								.$sqlcondition ;
       ...
       <tr>
        ...
        <a class="viewer_print" href="#" id="print" title="print"></a>
          ...
          <tr>
                ...
                ?> 
				<th colspan="2" class="col1_50">Final Grade</th>
			</tr>
        <?php
        ...
        ?>
            <tr class="<?=($x%2==0)?"":"highlight";?>">
                ...
                <td><?=getSubjName($row["subject_id"]).getStudSubjElecName($row['subject_id'],USER_STUDENT_ID)?></td>
                <?php
				$query_period = mysql_query($sql_period);
				$period_ctr = 1;
				while($row_period = mysql_fetch_array($query_period))
				{
				?>
                <td <?=checkIfGradeIsPass(getStudentGradePerPeriod($row["id"],$row_period['id'], USER_STUDENT_ID))==='N'?'style="color:red"':''?>><?=getStudentGradePerPeriod($row["id"],$row_period['id'],USER_STUDENT_ID)?></td>
              	<?php
				$period_ctr++;
				
				}
				
				$f_grade = getStudentFinalGrade(USER_STUDENT_ID,$row["id"],$term_id);
				
				if(checkIfSubjectDroppedByTerm(USER_STUDENT_ID,$row['id'],$term_id))
					{
				?>
                
              <td style="color:red"><?=getSubjectDropped(USER_STUDENT_ID,$row['id'],$term_id)=='D'?'DROPPED':'FAILED'?></td>
                <td style="color:red"><?=getSubjectDropped(USER_STUDENT_ID,$row['id'],$term_id)=='D'?'0.0':'5.0'?></td>
                
				<?php	
					}else if(checkIfSubjectINCByTerm(USER_STUDENT_ID,$row['id'],$term_id)){
				?>
             ...
             <?php
				...
				?>
                <td><?=$f_grade?></td>
                <td colspan="2" <?=checkIfGradeIsPass(getStudentFinalGrade(USER_STUDENT_ID,$row["id"],$term_id))==='N'?'style="color:red"':''?>>
                    <?php
						
                        if($f_grade != '')
                        {
                            echo getGradeConversionGrade2(USER_STUDENT_ID,$row["id"],$term_id);
                        }
                        else
                        {
                            echo '0.0';
