...
$(function(){
	...
	$("a[name='stud_id']").click(function(){
		...
		$('#box_container').load('ajax_components/ajax_com_st_pending_payment_history.php?student_id='+param, null);
		return false;
	});
...
if($ctr_reserve != '0')
{
	$sql_pagination = "SELECT DISTINCT student_id FROM tbl_student_reserve_subject WHERE student_id =".USER_STUDENT_ID 
							 .$sqlcondition ;
	...
	if($row_ctr>0)
	{
		...
		$sql = "SELECT DISTINCT student_id FROM tbl_student_reserve_subject"
							 . $sqlcondition  . $sqlOrderBy . " WHERE student_id =". USER_STUDENT_ID ." $max"  ;
