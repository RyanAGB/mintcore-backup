...
<?php
	...
	if(isset($fieldName) || $fieldName != '' )
	{
		...
		{
			$sqlOrderBy = " ORDER BY  employee.$fieldName ". $_REQUEST['orderBy'] ." , college.college_name ASC";
		}
		else
		{
	...
	else if($_SESSION[CORE_U_CODE]['fieldName']!='' || $_SESSION[CORE_U_CODE]['orderBy'] !='')
	{
		...
		{
			$sqlOrderBy = " ORDER BY  employee.".$_SESSION[CORE_U_CODE]['fieldName']." ". $_SESSION[CORE_U_CODE]['orderBy'] ." , college.college_name ASC";
		}
		else
		{
	...
	$sql_pagination = "SELECT college.* 
					FROM 
						tbl_college college,
						tbl_employee employee
					WHERE
						college.emp_id = employee.id
						" .$sqlcondition ;
...
?>                  
        ...
        <table class="listview">      
          <tr>
              <th class="col_20">&nbsp;</th>
            <th class="col_150"><a href="#" class="sortBy" returnFilter="college_name">College Code</a></th>
             <th class="col_150"><a href="#" class="sortBy" returnFilter="college_code">College Name</a></th>
              <th class="col_350"><a href="#" class="sortBy" returnFilter="lastname">College Dean</a></th>
        ...
        ?>
            <tr class="<?=($x%2==0)?"":"highlight";?>">
                ...
                if($canEdit =='Y')
				{
				?><a href="javascript:doTheAction ('edit', 'edit', 'id_<?=$row['id']?>');" class="edtlnk" val="<?=$row["id"]?>"><?=$row["college_code"]?></a>
                <?php
                }
				else
				{
					echo $row['college_code'];
				}
				?></td> 
                <td><?=$row["college_name"]?></td>
                <td><?=getProfessorFullName($row["emp_id"])?></td>
                <?php
                if($canEdit =='Y')
				{
				?>
