...
<?php	
	...
	$sql_pagination = "SELECT bib.*,col.description FROM ob_biblio bib,ob_collection_dm col
			WHERE bib.collection_cd=col.code
							" .$sqlcondition ;
	...
	if($row_ctr>0)
	{
		...
		$sql = "SELECT bib.*,col.description FROM ob_biblio bib,ob_collection_dm col
			WHERE bib.collection_cd=col.code
        ...
        <table class="listview">      
          <tr>
              <th class="col_70"><a href="#" class="sortBy" returnFilter="call_nmbr1">Call Number</a></th>
        ...
        ?>
            <tr class="<?=($x%2==0)?"":"highlight";?>">
                <td ><?=$row["call_nmbr1"].$row["call_nmbr2"].$row["call_nmbr3"]?></td> 
                <td><?=$row["title"]?></td>
                <td><?=$row['author']?></td>
