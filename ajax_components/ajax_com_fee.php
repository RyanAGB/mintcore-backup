...
<?php
	...
	$sql_pagination = "SELECT fee.*,year.id as sy_id, term.id as term_id 
							FROM tbl_school_fee fee, 
								tbl_school_year_term term, 
								tbl_school_year year 
							WHERE fee.term_id = term.id 
							AND year.id = term.school_year_id " .$sqlcondition ;
	...
	if($row_ctr>0)
	{
		...
		$sql = "SELECT fee.*,year.id as sy_id, term.id as term_id 
							FROM tbl_school_fee fee, 
								tbl_school_year_term term, 
								tbl_school_year year 
							WHERE fee.term_id = term.id 
							AND year.id = term.school_year_id " .$sqlcondition  . $sqlOrderBy . " $max" ;
...
?>                  
        ...
        <table class="listview">      
          <tr>
              <th class="col_20">&nbsp;</th>
            <th class="col_200"><a href="#" class="sortBy" returnFilter="fee_name">Fee Name</a></th>
              <th class="col_150"><a href="#" class="sortBy" returnFilter="start_year">School Year</a></th>
              <th class="col_50"><a href="#" class="sortBy" returnFilter="amount">Amount</a></th>
              <th class="col_150"><a href="#list" onclick="sortList('fee_type');">Fee Type</a></th>
        ...
        ?>
            <tr class="<?=($x%2==0)?"":"highlight";?>">
                ...
                <td><a href="javascript:doTheAction ('edit', 'edit', 'id_<?=$row['id']?>');" class="edtlnk" val="<?=$row["id"]?>"><?=$row["fee_name"]?></a></td> 
                <td><?=getSYandTerm($row['term_id'])?></td>
                <td><?=$row["amount"]?></td>
                <td><?=getFeeTypeName($row["fee_type"])?></td>
                <td class="action">
                    <ul>
