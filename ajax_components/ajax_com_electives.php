...
<?php
	...
	$sql_pagination = "SELECT * FROM tbl_curriculum_subject WHERE subject_category='E' OR subject_category='EO' " .$sqlcondition ;
	...
	if($row_ctr>0)
	{
		...
		$sql = "SELECT * FROM tbl_curriculum_subject WHERE subject_category='E' OR subject_category='EO' " .$sqlcondition  . $sqlOrderBy . " $max";
...
?>                  
        ...
        <table class="listview">      
          <tr>
              <th class="col_20">&nbsp;</th>
            <th class="col_350"><a href="#" class="sortBy" returnFilter="subject_code">Elective Name</a></th>
               <th class="col_350"><a href="#" class="sortBy" returnFilter="department_name">Subject Name</a></th>
        ...
        ?>
            <tr class="<?=($x%2==0)?"":"highlight";?>">
                <td ><input type="checkbox" name="id" id="id_<?=$row['subject_id']?>" value="<?=$row['subject_id']?>" /></td>
                <td><a href="javascript:doTheAction ('edit', 'edit', 'id_<?=$row['subject_id']?>');" class="edtlnk" val="<?=$row["subject_id"]?>"><?=getSubjName($row["subject_id"])?></a></td> 
                <td><?=getSubjName(getElectiveSubject($row["subject_id"]))?></td>
                <td class="action">
                    <ul>
                        <li><a class="edit" href="javascript:doTheAction ('edit', 'edit', 'id_<?=$row['subject_id']?>');" title="edit"></a></li>
