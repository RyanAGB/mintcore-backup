...
$sched = $_REQUEST['sched'];
	...
	$sql = "SELECT * FROM tbl_student_schedule 
			WHERE enrollment_status = 'A' AND student_id=".$id." AND term_id=".CURRENT_TERM_ID." AND schedule_id<>".$sched;
	...
	$(function(){
		...
		$('.selector').click(function() {
			...
			var subj = $(this).attr("returnSubjId");
					...
					if(confirm('Are you sure you want to change this subject? This will be considered enrolled.'))
									{
										...
										success: function(msg){
											if (msg != ''){
											//alert('yessss');
												$('#action').val('change_subject');
												$('#add_sub').val(ObjId);
												$('#ch_sub').val($('#change').val());
												$("form").submit();
...
<div id="lookup_content">
    ...
    <table class="fieldsetList" style="width:800px;">      
		...
		foreach($arr_subject as $subject_id) 
		{
			...
			while($row = mysql_fetch_array($result)) 
        	{ 
				if(!in_array($row['id'],$scheds))
				{
				if($row["id"]!=$sched)
				{
				$un = getSubjUnit($row["subject_id"]);
				if(getSubjUnit($row["subject_id"])==getSubjUnit($sub_id)&&getSubjType($row["subject_id"])==getSubjType($sub_id))
				//)
				{
        ?>
        ...
        ?>
        <input type="hidden" name="change" id="change" value="<?=$sub_id?>">
    </table> 
</div> <!-- #lookup_content -->    
<?php
}
