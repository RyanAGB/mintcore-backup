...
<script type="text/javascript">
	$(function(){
		...
		$('#pdfdec').click(function() {
			var w=window.open ("pdf_reports/rep107.php?id="+<?=$id?>+"&met=dec"); 
			return false;
		});
		
		$('#emaildec').click(function() {
			
			if(confirm("Are you sure you want to email this file to this student?"))
			{
				$.ajax({
					type: "POST",
					data: "id="+<?=$id?>+"&met=dec&email=1",
...
<?php
	...
	$sql = "SELECT * FROM tbl_decline_application WHERE id = $id";						
	$query = mysql_query($sql);
	$row = mysql_fetch_array($query);

	$sql_student_photo = "SELECT image_file FROM tbl_decline_application WHERE id = $id";
...
<tr>
    <td colspan=2 class="title-action">
   		<a class="viewer_email" href="#" id="emaildec" title="email"></a>
        <a class="viewer_pdf" href="#" id="pdfdec" title="pdf"></a>
        <a class="viewer_print" href="#" id="printdec" title="print"></a>
    </td>
</tr>
</table>
</div>
...
<table border="0" cellspacing="10" cellpadding="0">
  <tr>
    ...
    {
    ?>
        <img src="includes/getimage_decline.php?student_id=<?=$row['id']?>"/>
    <?php
    }
    else
    {
