	...
	class TCPDF {
	 ...
	 /**
		...
		*/
		...
		/*************************ORIG IMAGE******************/
		function Image2($file, $x, $y, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='') {
			// get image size
				...
				if (!$info) {
					if (function_exists($gdfunction)) {
						...
						if ($resize) {
							...
							$info = $this->_to2JPEG($imgr);
						} else {
							$info = $this->_to2JPEG($img);
						}
					} else {
						return;
					}
		...
		*/
		...
		function Image($file, $x, $y, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='') {
			...
			/*if (!isset($this->images[$file])) {
				...
				$mtd = '_parsejpeg';//.$type;
				// GD image handler function
				$gdfunction = "imagecreatefromstring";//.$type;
				$info = false;
				//if ((method_exists($this,$mtd)) AND (!($resize AND function_exists($gdfunction)))) {
		...
		*/
		function _toJPEG($image) {
			...
			imagejpeg($image, $tempname, $this->jpeg_quality);
			//imagedestroy($image);
			$retvars = $this->_parse2jpeg($tempname);
			// tidy up by removing temporary image
			unlink($tempname);
			return $retvars;
		}
		...
		*/
		function _putfonts() {
			...
			foreach($this->FontFiles as $file => $info) {
				...
				$font = file_get_contents('fonts/'.strtolower($file));
